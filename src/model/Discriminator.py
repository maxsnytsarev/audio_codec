...
class DiscriminatorBlock(nn.Module):
    def __init__(self, in_channels):
        ...
        self.conv1 = nn.Conv1d(
            in_channels=in_channels, out_channels=16, stride=1, kernel_size=15
        )
        self.lrelu = nn.LeakyReLU(0.2)
        self.blocks = nn.ModuleList()
        self.blocks.append(
            nn.Conv1d(
                in_channels=16, out_channels=64, stride=4, kernel_size=41, groups=4
            )
        )
        self.blocks.append(
            nn.Conv1d(
                in_channels=64, out_channels=256, stride=4, kernel_size=41, groups=16
            )
        )
        self.blocks.append(
            nn.Conv1d(
                in_channels=256, out_channels=1024, stride=4, kernel_size=41, groups=64
            )
        )
        self.blocks.append(
            nn.Conv1d(
        ...
        self.conv2 = nn.Conv1d(
            in_channels=1024, out_channels=1024, stride=1, kernel_size=5
        )
        self.conv3 = nn.Conv1d(
            in_channels=1024, out_channels=1, stride=1, kernel_size=3
        )
...
class STFTDiscriminator(nn.Module):
    def __init__(self):
        ...
        self.conv2 = nn.Conv2d(
            in_channels=16 * C, out_channels=1, kernel_size=(1, F // 2**6)
        )
